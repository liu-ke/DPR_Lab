...
input1_mean=mean(input1(:,5:9),1);              %compute the mean feature for input1 and input2 
input2_mean=mean(input2(:,5:9),1);

%outlier detection using Mahalanobis Distance
if method==1
    MD1=pdist2(input1(:,5:9),input1_mean,'mahalanobis');    %calculate Mahalanobis Distance between each feature and mean feature
...
if method==2
    [cov1,mu1,RD1,outliers1]=robustcov(input1(:,5:9));      %computer the Robust Distance for input1 and input2, using robust mean and robust covariance
	[cov2,mu2,RD2,outliers2]=robustcov(input2(:,5:9));      %threshold for outliers is default offered by robustcov() 
    %thres1=mean(RD1)*1.6;
    %thres2=mean(RD2)*1.6;
...
input2(outliers2==1,:)=[];
new_features1=[input1(:,1),input1(:,5:9)];                  %remove location information in inputs.
new_features2=[input2(:,1),input2(:,5:9)];